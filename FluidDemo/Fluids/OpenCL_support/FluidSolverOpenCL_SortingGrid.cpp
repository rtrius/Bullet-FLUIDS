/* FluidSolverOpenCL_SortingGrid.cpp
...
*/
#include "FluidSolverOpenCL_SortingGrid.h"
...
FluidSolverOpenCL_SortingGrid::FluidSolverOpenCL_SortingGrid()
{
...
void FluidSolverOpenCL_SortingGrid::initialize()
{
...
void FluidSolverOpenCL_SortingGrid::deactivate()
{
...
void FluidSolverOpenCL_SortingGrid::stepSimulation(const FluidParametersGlobal &FG, btAlignedObjectArray<FluidSph*> *fluids)
{	
	BT_PROFILE("FluidSolverOpenCL_SortingGrid::stepSimulation()");
	...
	//FluidSolverOpenCL_SortingGrid requires use of 'class FluidSortingGrid'
	...
	//stepSimulation()
	{
		...
		for(int i = 0; i < numValidFluids; ++i)
		{
			...
			sph_computePressure( numFluidParticles, &gridPointers, &fluidPointers, validFluids[i]->getGrid()->getCellSize() );
...
void FluidSolverOpenCL_SortingGrid::initialize_stage1_platform()
{	
	cl_int error_code;
...
void FluidSolverOpenCL_SortingGrid::initialize_stage2_device()
{
	cl_int error_code;
	const size_t MAX_STRING_LENGTH = 1024;
	char string[MAX_STRING_LENGTH];
	...
	{
		printf("FluidSolverOpenCL_SortingGrid::initialize_stage2_device() error: invalid platform_id. \n");
	}
}

void FluidSolverOpenCL_SortingGrid::initialize_stage3_context_and_queue()
{
	cl_int error_code;
	...
	{
		printf("FluidSolverOpenCL_SortingGrid::initialize_stage3_context_and_queue() error: invalid gpu_device. \n");
	}
}

void FluidSolverOpenCL_SortingGrid::initialize_stage4_program_and_buffer()
{
	cl_int error_code;
	...
	else printf("FluidSolverOpenCL_SortingGrid::initialize_stage4_program_and_buffers() error: invalid context or command_queue. \n");
}

void FluidSolverOpenCL_SortingGrid::deactivate_stage1_program_and_buffer()
{
	cl_int error_code;
...
void FluidSolverOpenCL_SortingGrid::deactivate_stage2_context_and_queue()
{
	cl_int error_code;
...
void FluidSolverOpenCL_SortingGrid::writeSingleFluidToOpenCL(FluidSph* fluid, Fluid_OpenCL *fluidData, FluidSortingGrid_OpenCL *gridData)
{
	const bool WRITE_ALL_DATA = false;
	const FluidParametersLocal &FL = fluid->getLocalParameters();
		
	gridData->writeToOpenCL( context, gpu_command_queue, reinterpret_cast<FluidSortingGrid*>( fluid->internalGetGrid() ) );
	fluidData->writeToOpenCL( context, gpu_command_queue, FL, &fluid->internalGetFluidParticles(), WRITE_ALL_DATA );
}
void FluidSolverOpenCL_SortingGrid::readSingleFluidFromOpenCL(FluidSph* fluid, Fluid_OpenCL *fluidData, FluidSortingGrid_OpenCL *gridData)
{	
	const bool READ_ALL_DATA = false;
	
	//	FluidSortingGrid::insertParticles() OpenCL program not implemented
	//gridData->readFromOpenCL( context, gpu_command_queue, reinterpret_cast<FluidSortingGrid*>( fluid->internalGetGrid() ) );
	fluidData->readFromOpenCL( context, gpu_command_queue, &fluid->internalGetFluidParticles(), READ_ALL_DATA );
}

void FluidSolverOpenCL_SortingGrid::sph_computePressure(int numFluidParticles, FluidSortingGrid_OpenCLPointers *gridPointers, 
														Fluid_OpenCLPointers *fluidPointers, btScalar cellSize) 
{
	BT_PROFILE("FluidSolverOpenCL_SortingGrid::sph_computePressure()");
	...
	CHECK_CL_ERROR(error_code);
	error_code = clSetKernelArg( kernel_sph_computePressure, 7, sizeof(void*), gridPointers->m_buffer_numActiveCells );
	CHECK_CL_ERROR(error_code);
	error_code = clSetKernelArg( kernel_sph_computePressure, 8, sizeof(void*), gridPointers->m_buffer_activeCells );
	CHECK_CL_ERROR(error_code);
	error_code = clSetKernelArg( kernel_sph_computePressure, 9, sizeof(void*), gridPointers->m_buffer_cellContents );
	CHECK_CL_ERROR(error_code);
	error_code = clSetKernelArg( kernel_sph_computePressure, 10, sizeof(btScalar), static_cast<void*>(&cellSize) );
	CHECK_CL_ERROR(error_code);
...
void FluidSolverOpenCL_SortingGrid::sph_computeForce(int numFluidParticles, FluidSortingGrid_OpenCLPointers *gridPointers, Fluid_OpenCLPointers *fluidPointers) 
{
	BT_PROFILE("FluidSolverOpenCL_SortingGrid::sph_computeForce()");
...
void FluidSolverOpenCL_SortingGrid::integrate(int numFluidParticles, Fluid_OpenCLPointers *fluidPointers) 
{
	BT_PROFILE("FluidSolverOpenCL_SortingGrid::integrate()");
